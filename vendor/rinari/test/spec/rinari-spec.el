...
(setq rinari-behave-root 
      (if load-file-name
	  (expand-file-name (concat (file-name-directory load-file-name) "/../sample/"))
	(expand-file-name (concat (buffer-file-name (current-buffer)) "/../sample/"))))
...
(context "rhtml navigation should"
	 ...
	 (lexical-let ((file (concat rinari-behave-root "app/views/foo/bar.rhtml"))
		       (controller-file (concat rinari-behave-root "app/controllers/foo_controller.rb"))
		       (model-file (concat rinari-behave-root "app/models/foo.rb")))
...
;		    (find-file file)
;		    (rinari-toggle-action)
;		    (expect (file-name-nondirectory (buffer-file-name (current-buffer))) equals "foo_controller.rb")
;		    (kill-buffer (current-buffer)))

	   (specify "toggle to/from tests in controller"
		    (find-file controller-file)
		    (toggle-buffer)
		    (expect (file-name-nondirectory (buffer-file-name (current-buffer))) 
			    equals "foo_controller_test.rb")
		    (toggle-buffer)
		    (expect (buffer-file-name (current-buffer))
			    equals controller-file))

	   (specify "toggle to/from tests in model"
		    (find-file model-file)
		    (toggle-buffer)
		    (expect (file-name-nondirectory (buffer-file-name (current-buffer))) 
			    equals "foo_test.rb")
		    (toggle-buffer)
	   ...
	   (specify "toggle by non-default rules"
		    (find-file controller-file)
		    (toggle-buffer 'zentest)
		    (expect (buffer-file-name (current-buffer))
			    equals (concat rinari-behave-root "test/controllers/foo_controller_test.rb"))
		    (kill-buffer (current-buffer)))
